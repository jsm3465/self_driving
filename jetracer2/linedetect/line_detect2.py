...
class LineDetector:
    ...
    def getPresentRoad(self, roi_img):
        histogramleft = np.sum(roi_img[:, :roi_img.shape[1] // 2], axis=1)
        histogramright = np.sum(roi_img[:, roi_img.shape[1] // 2:], axis=1)
        ...
        for i in histogramright:
            if rightflag == 0 and i > 1000:
                ...
                cnt += 1
                if cnt > 50:
                    self.rover.presentroad = 1

        # print("right cnt : ", cnt)
        print("현재 차선 :", self.rover.presentroad, "차선")
    ...
    def line_detect(self, frame):
        ...
        lines = cv2.HoughLinesP(roi_img, 1, np.pi / 180, 30, minLineLength=10, maxLineGap=20)
    ...
    def line_camera(self, frame):
        ...
        self.rover.L_lines = L_lines
        self.rover.R_lines = R_lines
