...
class LaneDetection:
    ...
    def linedetect(self, birdeye, blur):
        ...
        if len(nonzero_left[0]) > 4500*0.3:
            ...
            if (nonzero_left[0].max() - nonzero_left[0].min()) > bird_height*0.3:
                ...
                if up * down > 0:
                    ...
                    print("Left", self.offcenter)

        # 오른쪽 차선
        nonzero_right = np.nonzero(right_blur[10:])
        rmid = 0
        if len(nonzero_right[0]) > 4500*0.3:
